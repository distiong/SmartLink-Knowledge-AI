...
Write-Host "AI智能知识库问答系统 - 启动中..." -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
mysql -u root -p123456 -e "USE knowledge_base; CREATE TABLE IF NOT EXISTS ai_document (id BIGINT AUTO_INCREMENT PRIMARY KEY, file_name VARCHAR(255) NOT NULL, file_type VARCHAR(50) NOT NULL, content TEXT, create_time DATETIME DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;" 2>&1 | Out-Null
...
Write-Host "等待Milvus服务启动 (15秒)..." -ForegroundColor Yellow
Start-Sleep -Seconds 15

# 启动后端
Write-Host "[4/4] 启动后端服务 (端口 8080)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "`$env:JAVA_HOME='D:\softtware\java17'; cd 'E:\HuaYuWork\AIdemo\backend'; mvn spring-boot:run"
